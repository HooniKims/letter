...
- Apps Script `doPost`에서 `LockService.getScriptLock()`을 사용한다.
- 여러 학생이 동시에 `최종 완성하기`를 눌러도 한 번에 하나의 저장 작업만 시트에 쓰도록 잠근다.
- 잠금을 얻은 요청만 `appendRow([학번 이름, 편지 내용, 생성일시])`를 실행한다.
- 저장 후 방금 추가한 행의 B열 줄바꿈과 C열 가운데 정렬을 적용한다.
- 잠금은 `finally`에서 항상 해제한다.
- 잠금 대기 시간은 `30000ms`로 설정했다.
- 30초 안에 잠금을 얻지 못하면 저장 실패 응답이 반환되고, 사용자는 다시 시도해야 한다.
...
### LM Studio 규칙 문장 출력 방지

문제:

- 로컬 모델이 최종 카드 문구 대신 아래와 같은 프롬프트 규칙 일부만 출력하는 경우가 있었다.

```text
엄마 아빠,' 또는 '부모님,'으로 시작. * 존댓말 사용 (-요, -게요, -습니다).
```

원인:

- 프롬프트 규칙에 `'엄마 아빠,'`, `'부모님,'`처럼 따옴표로 감싼 실제 시작 문구 예시가 들어 있었다.
- Gemma 계열 로컬 모델이 이 규칙 줄을 최종 답변처럼 복사할 수 있었다.
- 브라우저 직접 호출용 정규화 함수가 따옴표 안의 예시 문구도 카드 시작 후보로 잘못 인식했다.

수정:

- 클라이언트 프롬프트와 서버 프롬프트에서 따옴표로 감싼 시작 문구 예시를 제거했다.
- `public/js/letterPrompt.js`에서 따옴표 안의 주소 후보와 `첫 문장`, `존댓말`, `시작`, `또는` 같은 지시문 문맥을 카드 본문 후보에서 제외했다.
- 규칙 설명만 나온 응답은 빈 결과로 처리해 화면에 그대로 표시되지 않게 했다.
- 정규화된 결과가 `엄마 아빠,` 또는 `부모님,`으로 직접 시작하지 않으면 카드 문구로 인정하지 않도록 했다.
- 이 경우 브라우저 LM Studio 직접 호출은 실패로 간주되고 기존 OpenAI fallback 경로로 넘어간다.

검증:

- `tests/client-lmstudio.test.mjs`에 규칙 문장만 출력된 경우와 규칙 뒤 실제 카드가 붙은 경우의 회귀 테스트를 추가했다.
- `요구사항 분석`처럼 주소 없이 시작하는 메타 문장도 빈 결과로 처리되는 테스트를 추가했다.
- 프롬프트에 따옴표로 감싼 주소 예시가 다시 들어가지 않도록 테스트를 추가했다.
- 실제 LM Studio 브라우저형 호출에서 `provider=lmstudio-browser`, `model=google/gemma-4-e2b`로 생성되고 규칙 문장이 섞이지 않는 것을 확인했다.
